...
| 主題 | RV 2nd | M4 | QA6 / 補充 | 大碩 |
|---|---|---|---|---|
| Logic design conventions | §4.2 | §4.2 | — | 附錄 A（RV）/ 附錄 C（M4） |
| ⭐ Flip-flop timing（$T_{setup}$ / $T_{hold}$ / $T_{C2Q}$）、sequential circuit max freq | §4.2 / Appendix A | §4.2 / 附錄 C | **Mano Ch5 §5.4–5.5** | — |
...
| 主題 | RV 2nd | M4 | QA6 / 補充 | 大碩 |
|---|---|---|---|---|
| ⭐ Pipelining overview（IF/ID/EX/MEM/WB） | §4.5 | §4.5 | QA6 Appendix C.1 | Ch5 重點1 |
| ⭐ Pipelined datapath + control | §4.6 | §4.6 | — | Ch5 重點2–3 |
| ⭐ Data hazards: forwarding / stalling / load-use | §4.7 | §4.7 | QA6 Appendix C.2 | Ch5 重點4–7 |
| ⭐🔶 Control hazards | §4.8 | §4.8 | — | Ch5 重點8 |
| ⭐ Branch prediction（BHT 1-bit / 2-bit / correlating） | §4.8 內 | §4.8 內 | **QA6 Ch3 §3.3–3.4**（tournament, gshare） | Ch5 重點8 |
| Exceptions in pipeline | §4.9 | §4.9 | — | Ch5 重點10 |
| 🔷 ILP: superscalar, dynamic scheduling, OoO | §4.10 | §4.10 | **QA6 Ch3 §3.4–3.9**（Tomasulo, ROB, speculation） | Ch5 重點9 |
| 🔷 Super-pipelining / VLIW | — | — | **QA6 Ch3 §3.2（VLIW）, Appendix H** | — |
| Real Stuff pipeline | §4.11（🆕 ARM Cortex-A53 + i7） | §4.11（Opteron X4） | QA6 §3.12（i7 detailed pipeline） | — |
...
| ✅ Memory technologies（SRAM/DRAM/Flash 集中講） | §5.2 | 散在 I/O 章 | QA6 Appendix B（Memory Hierarchy Review） | — |
...
| Cache bandwidth: non-blocking (lockup-free), multi-banking, prefetch | §5.4 內 | §5.3 內 | **QA6 Ch2 §2.3**（10 advanced optimizations） | — |
| 🔷 Advanced cache optimizations（way prediction, victim cache, HW prefetch, compiler opt） | — | — | **QA6 Ch2 §2.3** | — |
| Cache coherence（snooping, MESI） | §5.10 | §5.8 | **QA6 Ch5 §5.2–5.4**（directory-based, MOESI） | Ch8 重點5 |
...
| ⭐ Hardware multithreading（fine/coarse-grained, SMT） | §6.4 | §7.5 | **QA6 Ch3 §3.12**（SMT in i7） | Ch8 重點6 |
| Multicore / shared-memory multiprocessor | §6.5 | §7.3 | **QA6 Ch5 §5.1–5.3** | Ch8 重點3 |
| 🔷 Memory consistency models（SC, TSO, relaxed） | — | — | **QA6 Ch5 §5.6** | — |
| Message-passing / MPP / clusters / WSC | §6.7（🆕 WSC 擴充） | §7.4 | **QA6 Ch6**（WSC 完整章） | Ch8 重點4 |
| Cache coherence（跨接 §G） | §5.10 | §5.8 | **QA6 Ch5 §5.2–5.4**（directory-based） | Ch8 重點5 |
...
| 主題 | RV 2nd | M4 | QA6 / NVIDIA 文件 |
|---|---|---|---|
| ⭐ Intro to GPU（正文） | §6.6 | §7.7 | — |
| GPU system architectures | 附錄 B.2 | 附錄 A.2 | — |
| ⭐ Programming GPUs（CUDA model） | 附錄 B.3 | 附錄 A.3 | **CUDA Guide Ch1–5** |
| ⭐ Multithreaded multiprocessor architecture（SM, warp） | 附錄 B.4 | 附錄 A.4 | **QA6 Ch4 §4.4**（GPU arch 深入） |
| Parallel memory system | 附錄 B.5 | 附錄 A.5 | **CUDA Guide Ch5**（memory hierarchy） |
...
| 🔷 SIMD / Vector Processors（GPU 的前身觀念） | — | — | **QA6 Ch4 §4.1–4.3**（vector arch, SIMD extensions） |
| 🔷 DNN accelerators（TPU, systolic array） | — | — | **QA6 Ch7 §7.3–7.4**（Google TPU, DNN hardware） |
| 🔷 NVIDIA H100 / Hopper architecture | — | — | **H100 WP**（SM 架構, NVLink 4.0, HBM3, Tensor Core） |
| 🔷 NVIDIA GH200 Grace Hopper Superchip | — | — | **GH200 WP**（統一記憶體, NVLink-C2C, 72GB HBM3） |
| 🔷 MLPerf / AI inference benchmarks | — | — | **QA6 Ch7 §7.7** + NVIDIA 官方 MLPerf 結果 |
...
| 主題 | RV 2nd | M4 | QA6 / 補充 | 大碩 |
|---|---|---|---|---|
| Network topologies（mesh, torus, hypercube） | §6.8 | §7.8 | **QA6 Appendix F**（interconnect 完整附錄） | Ch8 重點9 |
| 🔷 Bisection bandwidth / diameter / routing | — | — | **QA6 Appendix F.2–F.4** | — |
| Cluster networking | §6.9 | — | — | — |
| ⭐ Roofline model（主要考台大） | §6.10–6.11 | §7.10–7.11 | **QA6 Ch4 §4.7**（Roofline for GPU） | — |
